...
namespace DepartmentsEmployees2.Data
{
    /// <summary>
            ...
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        int departmentId = reader.GetInt32(departmentIdColumnPosition);
            ...
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        var employeeDepartment = employeeDepartmentRepo.GetDepartmentById(departmentId);
            ...
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"UPDATE Employee
                                     ...
                                     WHERE Id = @id";
                    cmd.Parameters.Add(new SqlParameter("@FisrtName", employee.FirstName));
                    cmd.Parameters.Add(new SqlParameter("@LastName", employee.LastName));
                    cmd.Parameters.Add(new SqlParameter("@id", id));
                    cmd.Parameters.Add(new SqlParameter("@DepartmentId", employee.DepartmentId));
                    cmd.ExecuteNonQuery();
